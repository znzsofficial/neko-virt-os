...
| 灯光预设 stage/soft/contrast | ✅ | store + stage |
...
| M11 | face/morph 动作可选一条 | 无完整 morph 浏览器 |
| M12 | 模型列表：显隐 / 移除 / 切换当前动作 | canvas 列表，条目少 |
| M13 | 简单模型位移（射线点地放置或轴约束拖） | 不做完整 Gizmo |
| M14 | 空态 / 错误态文案（中英若项目已有 i18n） | 加载中、格式不支持、OOM 风险提示 |
...
| ID | 候选效果 | 条件 |
|----|----------|------|
| V1 | 简单色调 / exposure | 极低成本，优先 |
| V2 | 轻 bloom 或仅 emissive 增强 | 仅高档；有开关；掉帧则砍 |
| V3 | 简单天空色 / 渐变背景 | 非完整 HDR 管线也可先做 |
| V4 | 可选 HDR/IBL（低分辨率 PMREM） | 手测后；低档关 |
| V5 | 雾 / 暗角 | 可选氛围；注意立体舒适度 |
...
1. **M8**：Quest 真机帧率 / 行走晕动签字（记 §6 表）  
2. **M6 / M10–M14**：有限多模型、会话清单、错误态（XR 内体验打磨）  
3. **M20–M23**：重置视角、进度条、灯光预设、可选地面阴影  
4. **V1–V3**：按需 **可关** 轻量视觉，手测不过就砍  
5. **X\***：物理 / 双柄等保持低优先级  

**已完成可跳过：** M0–M5、M7（会话骨架、舞台、加载、播放、行走、画质细项、Studio 预选）
