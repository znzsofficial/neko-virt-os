...
| 灯光 | 已有 | 平行光 + **环境光注入 MMD toon**（MeshToon 不吃场景 AmbientLight） |
| 项目系统 | 已有 | 列表、保存/打开、autosave（编辑器内 8s 防抖；录制/导出/busy 暂停）、关窗确认 |
| 导出 | 已有 | 默认 **离线 WebCodecs** + MediaRecorder；MP4/WebM；竖屏/自定义分辨率；**自定义视频 Mbps / 音频 kbps**；PNG/序列 ZIP |
| 关于页 | 已有 | 开源依赖许可表 |
...
src/appModules/mmdStudio/
  ...
  mmdSceneHydrate.ts
  mmdProjectDb.ts / mmdProjectIO.ts / mmdZipStore.ts
  mmdRuntime.ts                 # 运行时 + 物理步进约定
...
- 有模型或导出中关 MMD 窗 → `confirmCloseMmdStudio`；**不** flush autosave
...
物理回归手测（缩放 + 重置 + 多模型）
 ...
 → B1 多 VMD 权重叠层
 → C 舞台模型
 → 文档/应用描述文案若仍有旧「仅 WebM」表述则一并改
```

## 手测清单（回归）

1. 整文件夹导入 PMX → 贴图正常  
2. 保存 → 重开项目 → 贴图/动作/灯光恢复  
...
8. 离线导出 MP4/WebM、自定义分辨率、可选音频  
9. 关 MMD 窗确认；autosave 不在录制中刷盘  

## 本轮关键文件（物理 / 导出 / 灯光）

- `mmdPhysics.ts` / `mmdRuntime.ts` / `mmdRuntimeEntry.ts` — 物理 world、scale、seek
- `mmdWebCodecsExport.ts` / `useMmdRecordingController.ts` — 离线导出、取消、码率
- `MmdCanvas.tsx` — seekTime、导出 buffer、阴影、aspect
- `MmdPostFx.tsx` — composer 逻辑尺寸、MSAA/HalfFloat 降档
- `mmdMaterialEnhance.ts` — 环境光注入
- `mmdStudioStore.ts` / `MmdSidePanel.tsx` — 自定义码率 UI  
